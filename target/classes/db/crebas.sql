...
(
    ...
    passenger_name    varchar(20) comment '乘客人姓名',
...
(
    ...
    space_discount    decimal(3, 1) comment '舱位对应折扣',
...
(
    ...
    user_certype varchar(20) comment '证件类型',
    user_chname  varchar(20) comment '中文名',
...
(
    ...
    user_id          bigint comment '用户编号',
    userpass_name    varchar(20) comment '乘机人姓名',
...
(
    ticket_relyOnId     bigint not null,
    ticket_relyOnNumber bigint,
    primary key (ticket_relyOnId)
) CHARSET = utf8
  ENGINE = InnoDB;
...
/*==============================================================*/
/* View: v_airroute_airport_city                                */
/*==============================================================*/

create view v_airroute_airport_city(airroute_id, airportUpName, airportDownName, cityStartName, cityEndName,
                                    airroute_length)
as
...
alter table tb_airport
    add constraint FK_Relationship_1 foreign key (city_id)
        references tb_city (city_id) on delete restrict on update restrict;

alter table tb_airroute
    add constraint FK_Relationship_2 foreign key (airport_down)
        references tb_airport (airport_id) on delete restrict on update restrict;

alter table tb_airroute
    add constraint FK_Relationship_3 foreign key (airport_up)
        references tb_airport (airport_id) on delete restrict on update restrict;

alter table tb_flight
    add constraint FK_Relationship_5 foreign key (flights_id)
        references tb_flights (flights_id) on delete restrict on update restrict;

alter table tb_flights
    add constraint FK_Relationship_4 foreign key (airroute_id)
        references tb_airroute (airroute_id) on delete restrict on update restrict;

alter table tb_flights
    add constraint FK_Relationship_6 foreign key (model_id)
        references tb_model (model_id) on delete restrict on update restrict;

alter table tb_mdsp
    add constraint FK_tb_mdsp foreign key (model_id)
        references tb_model (model_id) on delete restrict on update restrict;

alter table tb_mdsp
    add constraint FK_tb_mdsp2 foreign key (space_id)
        references tb_space (space_id) on delete restrict on update restrict;

alter table tb_order
    add constraint FK_Relationship_10 foreign key (user_id)
        references tb_user (user_id) on delete restrict on update restrict;

alter table tb_passenger
    add constraint FK_Relationship_12 foreign key (ticket_id)
        references tb_ticket (ticket_id) on delete restrict on update restrict;

alter table tb_rule
    add constraint FK_Relationship_7 foreign key (space_id)
        references tb_space (space_id) on delete restrict on update restrict;

alter table tb_ticket
    add constraint FK_Relationship_11 foreign key (tickets_id)
        references tb_tickets (tickets_id) on delete restrict on update restrict;

alter table tb_ticket
    add constraint FK_Relationship_14 foreign key (order_id)
        references tb_order (order_id) on delete restrict on update restrict;

alter table tb_tickets
    add constraint FK_Relationship_8 foreign key (flight_id)
        references tb_flight (flight_id) on delete restrict on update restrict;

alter table tb_tickets
    add constraint FK_Relationship_9 foreign key (space_id)
        references tb_space (space_id) on delete restrict on update restrict;

alter table tb_userpass
    add constraint FK_Relationship_13 foreign key (user_id)
        references tb_user (user_id) on delete restrict on update restrict;
...
# 各id编码设定：
# order_id = "OD" + order_sureTime + user_id%10000(%04d) （共20位）
# flights_id =  自设定 (6位)
# flight_id = flights_id + flight_date    （共14位）
